...
export class AdhocListComponent implements OnInit {
  ...
  toggleStatus(eventchecked: any, params: any) {
    ...
    this.commonService.post('adhoc/' + params.id, params)
      .subscribe(
        details => {
          setTimeout(() => {this.authenticationService.loaderEnd();}, 10);
          if (details.success) {
  ...
  deleteAction(params: any) {
    this.commonService.get('adhoc/delete/' + params, {}).pipe().subscribe(
      data => {
        setTimeout(() => {this.authenticationService.loaderEnd();}, 10);
        if (data.success) {
          this.toastr.successToastr('Adhoc deleted sucessfully');
          this.getList();
        }
      });
  }
